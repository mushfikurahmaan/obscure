...
#[command]
fn save_data(app: AppHandle, master_password: String, data: String) -> Result<(), String> {
    ...
    // File format: salt || nonce || ciphertext (all base64, separated by ":")
...
#[command]
fn import_data(app: AppHandle, file_content: String) -> Result<(), String> {
    let path = get_data_file_path(&app);
...
pub fn run() {
  tauri::Builder::default()
    .invoke_handler(tauri::generate_handler![save_data, load_data, export_data, import_data, has_data_file])
    .setup(|app| {
